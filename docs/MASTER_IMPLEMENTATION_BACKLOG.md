...
7. `P3-007` `EKLE` priority aging, resource semaphore, bounded backpressure, micro-batch, durable cursor, token-bucket ve full-jitter retry algoritmalari. Durum: `DEVAM`.
...
Faz 3 kismi kanit: 2026-06-25 kosusunda `xnative/db/repositories.py` generic job enqueue/dedupe, active dedupe, bounded backpressure, priority aging, resource-class admission limit, expired lease recovery, retry/dead gecisi, dead-letter yazimi, durable cursor ve versioned cache repository davranislarini destekleyecek sekilde genisletildi. `xnative/db/migrations/0002_worker_runtime.sql` incremental cursor ve cache tablolarini ekler. `xnative/worker/scheduler.py` savepoint izolasyonlu tek is runner ve kontrollu `run_worker_loop` saglar; `xnative/worker/jobs.py` `normalize_capture` handler'ina sahiptir. `tests/integration/test_phase3_jobs.py` completed normalize job, lease recovery, retry -> dead-letter, unknown job type, resource semaphore, priority aging/live is sirasi, backpressure, cursor/cache ve handler hata rollback davranisini dogrular. Kalanlar: tam pipeline stage zinciri, yeniden oynatma UI aksiyonu, token-bucket/micro-batch ayrintisi ve dead-letter kaydinin UI'dan gorunmesi.
