...
2. `P4-002` `EKLE` dHash/pHash, Hamming threshold ve thumbnail tabanli benzer medya cluster testi. Durum: `TAMAM`.
3. `P4-003` `EKLE` varsayilan metadata-only arsiv: X URL, gorunur metin, alt text, metrik snapshot ve provenance. Durum: `TAMAM`.
4. `P4-004` `EKLE` opsiyonel yerel thumbnail/orijinal medya politikalari; telif/izin ve disk kotasi. Durum: `TAMAM`.
5. `P4-005` `EKLE` content-addressed media store, referans sayimi, TTL, LRU ve garbage collection. Durum: `TAMAM`.
6. `P4-006` `EKLE` bozuk/deleted URL durumu ve minimum kanit snapshot'i. Durum: `TAMAM`.
7. `P4-007` `EKLE` video icin sinirli frame sampling, audio extraction ve duration limiti. Durum: `TAMAM`.
...
Faz 4 kod kaniti: 2026-06-30 kosusunda `xnative/media/phash.py` eski SHA-256 prefix davranisindan ayrildi. `exact_sha256_file()` tam SHA-256 digest'i exact byte-level dedup icin, `difference_hash_file()` ise `dhash64-v1:<hex>` formatinda gercek perceptual hash icin kullanilir. `media_hashes_file()` ve `store_local_media()` exact/perceptual hash alanlarini ayri dondurur; backward-compatible `phash` alani perceptual hash'e esitlenir. `xnative/media/media_store.py` content-addressed path, manifest tabanli referans sayimi, duplicate blob engelleme, referans release, unreferenced media GC, retention TTL sonrasi metadata-only gecis, quota/LRU deletion policy, protected original policy ve silinmis remote URL snapshot davranisini saglar. `xnative/db/migrations/0004_media_lifecycle.sql` ve `UnitOfWork.media_lifecycle` kalici `local_media_objects`, `local_media_references` ve `remote_media_snapshots` kayitlarini ekler. `xnative/media/frame_sampler.py` video/gif icin sinirli frame offset plani, video/audio duration gate ve sinirli audio extraction karari uretir; gercek decode/OCR/ASR islemi Faz 5 adapterlarina birakilir. `tests/unit/test_media_hashing.py` ayni byte fixture'da SHA esitligini, kucuk gorsel degisiklikte exact SHA farkli ama dHash Hamming mesafesi yakin davranisini, farkli gorselde uzak davranisini, kucuk batch cluster sonucunu, tek fiziksel dosyada iki referans davranisini, referanssiz dosya GC'sini, TTL metadata-only gecisini, quota/LRU policy davranisini ve remote snapshot kaydini dogrular. `tests/unit/test_video_audio_lifecycle.py` bounded frame offset, uzun/missing duration reject, GIF audio skip, audio duration gate ve deterministic frame marker davranisini dogrular. `tests/integration/test_phase1_storage.py` DB lifecycle repository refcount, duplicate reference, release, remote snapshot idempotency ve `audit_log` uzerinden media lifecycle audit eventlerini dogrular. Kalan P4 uygulama isi yok; Faz 5 medya anlama adapterlari bu lifecycle ciktisini tuketecek.

### Faz 5 - Multimodal icerik anlama

Bagimlilik: Faz 3 ve 4.

1. `P5-001` `EKLE` Turkce/çokdilli metin normalization, dil tespiti, entity ve konu adaylari. Durum: `DEVAM`.
2. `P5-002` `EKLE` OCR adapteri: Tesseract/EasyOCR secilebilir; alt text fallback; confidence ve bounding box saklama. Durum: `DEVAM`.
...
9. `P5-009` `EKLE` 8 GB RAM/GTX 1050 hardware preflight, CPU fallback, tek-agir-model scheduler ve OOM recovery. Durum: `DEVAM`.
...
Faz 5 kismi kanit: `xnative/media/multimodal_evidence.py`, text/quote/visual/audio-video modality kanitlarini, topic adaylarini, OCR/alt-text fallback durumunu, missing modality bilgisini ve text-media iliski sinyalini zorunlu dis servis olmadan uretir. `tests/unit/test_multimodal_evidence.py`, futbol kelimesi metinde yokken gorsel/alt/OCR kanitinin konu saglamasini, eksik medyanin negatif sayilmamasini, video OCR/audio-video sinyalini ve hem legacy capture dataclass hem domain `CapturedPost` adapter davranisini dogrular. `xnative/runtime/hardware.py`, 8 GB RAM/GTX 1050 sinifi hedef icin RSS soft limit, CPU fallback, GPU/VRAM gate, tek agir model concurrency ve unload/defer kararini aciklanabilir hale getirir. `xnative-preflight`, hedef makinede agir model yuklemeden JSON/text rapor ve optional heavy-model admission dry-run uretir. `tests/unit/test_hardware_preflight.py`, GPU bilinmezken CPU fallback, RSS soft limit asimi, GPU kabul, CPU fallback kapali reject, RAM tahmini soft limit asimi, preflight rapor dosyasi ve bloklanan admission cikis kodu davranisini dogrular. Bu kanit henuz embedding, CLIP, ASR, confidence calibration modeli veya gercek soak benchmark'ini kapsamaz.
